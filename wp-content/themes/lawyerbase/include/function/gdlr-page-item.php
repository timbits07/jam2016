<?php
	...
	*/
		...
		function gdlr_get_item_title( $atts ){
			...
			if( !empty($atts['title-type']) && $atts['title-type'] != 'none' && 
				...
				if(!empty($atts['right-text']) && !empty($atts['right-text-link'])){
					$ret .= '<a class="gdlr-item-title-link" href="' . esc_url($atts['right-text-link']) . '" >' . $atts['right-text'] . '<i class="fa fa-long-arrow-right icon-long-arrow-right" ></i></a>';
				}
	...
	if( !function_exists('gdlr_get_service_half_background') ){
		function gdlr_get_service_half_background( $settings ){
			$item_id = empty($settings['page-item-id'])? '': ' id="' . $settings['page-item-id'] . '" ';
			...
			$ret  = '<div class="gdlr-service-half-background-item" ' . $item_id . $margin_style . '>';

			if( !empty($settings['left-bg-color']) ){
				$style = 'style="background: ' . $settings['left-bg-color'] . ';"';
			}
			$ret .= '<div class="gdlr-half-left" ' . $style . ' >';
			$ret .= '<div class="half-container">';
			$ret .= '<div class="gdlr-item-margin">';
			$ret .= '<div class="gdlr-service-half-background-left-head">';
			if( !empty($settings['left-icon']) ){
				$ret .= '<div class="service-half-background-icon" >';
				$ret .= '<i class="fa ' . $settings['left-icon'] . '" ></i>';
				$ret .= '</div>';
			}
			$ret .= '<h3 class="gdlr-left-service-title" >' . gdlr_text_filter($settings['left-title']) . '</h3>';
			$ret .= '</div>';
			$ret .= '<div class="gdlr-left-service-content" >' . gdlr_content_filter($settings['left-content']) . '</div>';
			$ret .= '</div>'; // gdlr-item
			$ret .= '</div>'; // half-container
			$ret .= '</div>'; // half-left
			
			if( !empty($settings['right-bg-image']) ){
				if( is_numeric($settings['right-bg-image']) ){
					$image_src = wp_get_attachment_image_src($settings['right-bg-image'], 'full');
					$style = 'style="background: url(\'' . $image_src[0] . '\') center 0px;"';
				}else{
					$style = 'style="background: url(\'' . $settings['right-bg-image'] . '\') center 0px;"';
				}
			}else{
				$style = '';
			}
			$ret .= '<div class="gdlr-half-right" ' . $style . ' >';
			$ret .= '<div class="half-container">';
			$ret .= '<div class="gdlr-item-margin">';
			$ret .= '<h3 class="gdlr-right-service-title" >' . gdlr_text_filter($settings['right-title']) . '</h3>';
			$ret .= '<div class="gdlr-right-service-caption" >' . gdlr_content_filter($settings['right-caption']) . '</div>';
			$ret .= '</div>'; // gdlr-item
			$ret .= '</div>'; // half-container
			$ret .= '</div>'; // half-right
			$ret .= '<div class="clear"></div>';
	...
	if( !function_exists('gdlr_get_stunning_text_item') ){
		function gdlr_get_stunning_text_item( $settings ){
			...
			$ret  = '<div class="gdlr-stunning-item-ux gdlr-ux">';
			$ret .= '<div class="gdlr-item gdlr-stunning-item' . $class . '" ' . $item_id . $margin_style . '>';
			$ret .= '<h2 class="stunning-item-title">' . gdlr_text_filter($settings['title']) . '</h2>';
			$ret .= '<div class="stunning-item-caption gdlr-skin-info gdlr-title-font">' . gdlr_content_filter($settings['caption']) . '</div>';
			if(!empty($settings['stunning-text-icon'])){
				$ret .= '<div class="stunning-item-icon"><i class="fa ' . $settings['stunning-text-icon'] . '"></i>';
				$ret .= '<div class="stunning-item-icon-divider gdlr-left"></div>';
				$ret .= '<div class="stunning-item-icon-divider gdlr-right"></div>';
				$ret .= '</div>';
			}
			$ret .= '</div>'; // gdlr-item
			$ret .= '</div>'; // gdlr-ux
	...
	if( !function_exists('gdlr_get_carousel_testimonial_item') ){
		function gdlr_get_carousel_testimonial_item( $settings ){	
			...
			foreach( $list as $tab ){ 
				...
				if( $settings['testimonial-style'] == 'modern-style' ){
					$ret .= '<div class="testimonial-author-image gdlr-skin-border" >';
					$ret .= gdlr_get_image($tab['gdl-tab-author-image'], 'thumbnail');
					$ret .= '</div>';
					
					$ret .= '<div class="testimonial-item-outer">';
					$ret .= '<div class="testimonial-item-inner gdlr-skin-box">';
					$ret .= '<div class="testimonial-content gdlr-skin-content">' . gdlr_content_filter($tab['gdl-tab-content']) . '</div>';
					$ret .= '</div>'; // testimonial-item-inner
					
					$ret .= '<div class="testimonial-info gdlr-title-font">';
					if( !empty($tab['gdl-tab-title'] ) ){
						$ret .= '<span class="testimonial-author gdlr-skin-link-color">' . gdlr_text_filter($tab['gdl-tab-title']) . '</span>';
					}
					if( !empty($tab['gdl-tab-position']) ){
						$ret .= '<span class="testimonial-position gdlr-skin-info">' . gdlr_text_filter($tab['gdl-tab-position']) . '</span>';
					}
					$ret .= '</div>'; // testimonial-info
					$ret .= '<div class="testimonial-gimmick" ></div>'; // testimonial-item-outer
					$ret .= '</div>'; // testimonial-item-outer
				}else{
					if( strpos($settings['testimonial-style'], 'plain-style') === false ){ // hide this in plain style
						$ret .= '<div class="testimonial-item-inner gdlr-skin-box">';
					}
